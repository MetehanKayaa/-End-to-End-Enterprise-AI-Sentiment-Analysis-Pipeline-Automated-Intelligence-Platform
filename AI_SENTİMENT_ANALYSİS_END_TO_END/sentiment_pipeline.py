...
class SentimentPipeline:
    ...
    def running_pipeline_on_a_record(self, phrase: str, max_retries: int = 2) -> int:
        """Classifies a single phrase: 1 (positive), 0 (neutral), -1 (negative)."""
    ...
    def iterating_over_a_column_dataset(self, column_name: str = "text", sample_size: int = 50, batch_size: int = 16):
        ...
        print(f"\nRunning sentiment analysis on {len(dataset):,} records (batch_size={batch_size})...")
    ...
    def generating_report(self, csv_path: str = None):
        ...
        if text_column is None or text_column not in df.columns:
            text_column = "text" if "text" in df.columns else df.select_dtypes(include="object").columns[0]
        ...
        sentiment_percentages = {k: round((v / total_rows) * 100, 2) for k, v in sentiment_counts.items()}
...
if __name__ == "__main__":
    ...
    data_ldr = DatasetLoader()
    pipeline = SentimentPipeline(model_mgr, data_ldr)

    # 50 satırlık test çıkarımı ve rapor üretimi
    pipeline.iterating_over_a_column_dataset(column_name="text", sample_size=50, batch_size=16)
    pipeline.generating_report()
