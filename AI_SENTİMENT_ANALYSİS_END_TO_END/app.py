...
# ─── session state ────────────────────────────────────────────────────────────
for _k, _v in {
    "device_type": None,  # 'cuda', 'mps', or 'cpu'
    "device_name": None,
    "model_manager": None,
    "dataset_loader": None,
    "pipeline": None,
...
with st.sidebar:
    ...
    dev = st.session_state.device_type

    dev_label = f"{st.session_state.device_name} ({dev.upper()})" if dev else "not checked"
    st.markdown(f"{_dot(dev)} Device: {dev_label}")
...
if page == "System Status":
    ...
    with col_hw:
        ...
        if st.button("Detect Compute Device"):
            try:
                import torch

                detected_device = gpu_preparation()
                st.session_state.device_type = detected_device

                if detected_device == "cuda":
                    st.session_state.device_name = torch.cuda.get_device_name(0)
                elif detected_device == "mps":
                    st.session_state.device_name = "Apple Silicon (Metal MPS)"
                else:
                    st.session_state.device_name = "Host CPU"

                st.rerun()
            except Exception as e:
                st.error(f"Error checking device: {e}")
        ...
        if dev == "cuda":
            ...
            st.success(f"CUDA GPU detected: **{st.session_state.device_name}**")
            ...
            st.success("Apple Silicon GPU Acceleration (**Metal Performance Shaders / MPS**) is active.")
            st.info("M4 unified memory architecture handles model weights directly via shared RAM.")
        elif dev == "cpu":
            st.warning("Running on CPU. Pipeline execution will be functional but slower.")
        else:
            st.info("Click 'Detect Compute Device' to check available acceleration (CUDA / Apple MPS / CPU).")

    with col_env:
        st.subheader("Environment & Packages")
        for pkg in ["torch", "transformers", "datasets", "accelerate", "bitsandbytes"]:
            try:
                ...
                if pkg == "bitsandbytes" and st.session_state.device_type == "mps":
                    st.markdown(f"ℹ️ **{pkg}** — *Not required for Apple Silicon (MPS)*")
                else:
                    st.markdown(f"❌ **{pkg}** — not installed")

        st.divider()
        hf_logged = os.getenv("HF_TOKEN") is not None
        if hf_logged:
            st.markdown("✅ Logged in to Hugging Face Hub (via `HF_TOKEN`)")
        else:
            st.markdown("ℹ️ Using local/default Hugging Face credentials cache")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    is_mps = st.session_state.device_type == "mps"
    use_quant = st.checkbox(
        "Enable 4-bit NF4 quantization (CUDA only)",
        value=not is_mps,
        disabled=is_mps,
        help="BitsAndBytes 4-bit is enabled on NVIDIA GPUs. Apple Silicon uses optimized FP16 directly.",
    )
    ...
    if st.button("Load Model", type="primary"):
        with st.spinner(f"Loading **{model_id}**…"):
            try:
                ...
                new_mm = ModelManager(model_id=model_id, use_quantization=use_quant)
                st.session_state.model_manager = new_mm
                st.session_state.device_type = new_mm.device
                st.session_state.pipeline = None  # Reset dependent pipeline
                st.success(f"Model successfully loaded: **{model_id}** on **{new_mm.device.upper()}**")
            except Exception as e:
                st.error(f"Failed to load model: {e}")

    if st.session_state.model_manager:
        loaded_mm = st.session_state.model_manager
        st.subheader("Loaded Model Configuration")
        st.write(f"**Model ID:** `{loaded_mm.model_id}`")
        st.write(f"**Active Device:** `{loaded_mm.device.upper()}`")
        st.write(f"**Quantization Active:** `{loaded_mm.use_quantization}`")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.pipeline is None:
        st.info("Pipeline not yet initialized. Click below to build it.")
    ...
    st.success("Pipeline is initialized and ready.")
    st.divider()
    ...
    output_csv = st.text_input(
        "Save annotated CSV to",
        value=str(Path("reports_csv_files") / "sentiment_debug_output.csv"),
    )
    ...
    if st.button("Run Sentiment Analysis", type="primary"):
        ...
        st.success(
            f"Finished {max_rows:,} rows — parse failures: **{failed}**. Saved to `{output_csv}`."
        )
    ...
    if st.button("Generate Report", type="primary"):
        ...
        with st.spinner("Generating report…"):
            try:
                ...
                if rdir:
                    st.info(f"Artifacts saved to `{rdir}` — view in **Browse Reports**.")
            except Exception as e:
